...
//
// 职责：
//   1. 巡检（每 5 秒，受「皮肤常驻」开关控制）：OpenCode 刷新/重启后皮肤丢失 →
//      等应用自己的主题落定（AMOLED 等在启动数秒后才应用，抢跑会收割到默认 dark）
//      → 重新收割 + 染色 + 注入
//   2. 端口探测：OpenCode 被普通方式重启（调试端口消失）时发 macOS 系统通知。
//      守护进程绝不主动重启 OpenCode——重启必须由用户执行 apply-skin.sh 发起
//   3. busy 让行：CLI 正在换肤（state.busyUntil 未过期）时跳过注入，避免双重染色
//
// 手动前台调试：node daemon.mjs --foreground
// 端口可用环境变量覆盖（测试场景）：OPENCODE_SKIN_CDP_PORT
...
import { loadTheme } from "./lib/themes.mjs";
import { applyRecipe, healthScript } from "./lib/core.mjs";

const here = dirname(fileURLToPath(import.meta.url));
const CDP_PORT = Number(process.env.OPENCODE_SKIN_CDP_PORT) || DEFAULT_PORT;
const POLL_MS = 5000;
...
if (isMain) {
  log(`daemon 已启动：巡检 CDP :${CDP_PORT} 每 ${POLL_MS / 1000}s`);
