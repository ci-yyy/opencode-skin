...
export const ZC_TO_OC = {
  card: "--v2-background-bg-base",             // 对话区卡片（局部实底）
  cardSelected: "--v2-background-bg-layer-02",
  backgroundAlt: "--v2-background-bg-layer-01",
  background: "--v2-background-bg-deep",       // 全窗底 ← 全透明值：zcode 分区设计，主区直接透图
...
// hero 主题（背景图/渐变）里「铺满全窗」的底色变量（bg-deep，实测 1280×800 全窗）。
// 分区设计（对齐 zcode 的布局语义）：
//   bg-deep（全窗）← colors.background（全透明 #00000000）→ 主区直接透图，
//     hero 主题没有 background 键时退回 sidebar 但降到 surfaceAlpha（默认 55%）
//   bg-base（对话区局部）← colors.card 原值 alpha（96% 实底，卡片可读）
//   其余 layer 系按原值，zcode 里它们本来就是局部层
function buildDirectCss(theme) {
  const hasHero = !!(theme.heroImageAbs || theme.heroCss);
  const heroAlpha = theme.surfaceAlpha ?? 0.55;
  const decls = [];
  const seen = new Set();
  for (const [zcKey, ocVar] of Object.entries(ZC_TO_OC)) {
    ...
    seen.add(ocVar);
    let out = value.trim();
    // 全窗底（bg-deep）在 hero 主题里统一压到半透：card 值直搬会盖死背景图
    if (hasHero && ocVar === "--v2-background-bg-deep" && zcKey !== "background") {
