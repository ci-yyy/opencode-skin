  ...
  return async function handleRequest(req, res) {
    ...
    try {
      ...
      if (req.method === "GET" && url.pathname === "/random") {
        ...
        await update({ theme: pick.dir, busyUntil: Date.now() + 8000 });
      ...
      if (appliedMatch && req.method === "POST") {
        ...
        // 面板拿到 CSS 后在自己页面里注入是异步的：置忙几秒让守护进程巡检让行，
        // 否则「上报完成 → 巡检看到皮肤不在 → 抢注入旧主题」把面板的选择顶掉
        await update({ theme: dir, busyUntil: Date.now() + 8000 });
        log?.(`面板：切换到「${dir}」`);
        return json(res, 200, { ok: true }, origin);
      }
