...
// 主窗口 = oc:// 协议页面。
// 零依赖：Node 22+ 自带全局 fetch 和 WebSocket。
...
// 页面目标分类：
//   main   = OpenCode 主窗口（oc:// 协议页面）
//   other  = 其他一切页面（devtools、内部页等）
// 严格限定 oc:// 前缀：http(s) 网页绝不当注入目标——那是浏览器页面，注入越界
export function classifyTargets(targets) {
  const pages = [];
  for (const target of Array.isArray(targets) ? targets : []) {
    if (target?.type !== "page") continue;
    const url = typeof target.url === "string" ? target.url : "";
    const kind = url.startsWith("oc://") ? "main" : "other";
...
export function pickMainWindow(pages) {
  ...
  return { target: main[0] || null, ambiguous: main.length > 1 };
}
