...
export const PANEL_LAUNCHER_ID = "ocsk-launcher";
export const PANEL_ROOT_SELECTOR = "[data-ocsk-root]";

export function domReadyScript() {
  return `(() => { try { return !!(document && (document.head || document.documentElement)); } catch { return false; } })()`;
}

// 面板整体移除：所有注入根元素带 data-ocsk-root 标记，统一清理
...
export function panelInjectionScript(panelSource, config) {
  return `window.__OCSK_CONFIG__ = ${JSON.stringify(config)};\n${panelSource}`;
}
...
export async function applyRecipe(session, recipe, opts = {}) {
  // 收割必须在干净状态：皮肤在场时收割到的是染色后的值，染上再加染。
  // dryRun 也不例外——先移除收割完再决定要不要注回
  ...
  if (opts.dryRun) {
    return { css: built.css, tinted: built.remapped ?? built.tinted, themeAttr: h.themeAttr };
  }
...
// 面板注入（apply.mjs --panel 用；daemon 走自己的会话缓存）
export async function injectPanelToApp(panelSource, config, { port = DEFAULT_PORT, waitMs = 15000 } = {}) {
  const target = await waitForMainWindow(port, waitMs);
  const session = await new CdpSession(target.webSocketDebuggerUrl).open();
  try {
    await waitDomReady(session, waitMs);
    return await session.evaluate(panelInjectionScript(panelSource, config));
  } finally {
    session.close();
  }
}

export async function removePanelFromApp({ port = DEFAULT_PORT, waitMs = 15000 } = {}) {
  const target = await waitForMainWindow(port, waitMs);
  const session = await new CdpSession(target.webSocketDebuggerUrl).open();
  try {
    return await session.evaluate(panelRemovalScript());
  } finally {
    session.close();
  }
}
